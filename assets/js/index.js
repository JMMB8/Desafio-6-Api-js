...
async function getMonedas() {
  try {
    const selectMoneda = document.getElementById("idMonedas").value;
    const endpoint = `https://mindicador.cl/api/${selectMoneda}`;
  ...
  try {
    ...
    if (!isNaN(valorMoneda)) {
      ...
      document.getElementById(
        "resultado"
      ).innerText = `${nombreMoneda} ${conversion.toFixed(2)}`;
    } else {
      document.getElementById("resultado").innerText = "Error en la conversión";
    }
...
document.getElementById("btnBuscar").addEventListener("click", async () => {
  ...
  const montoClp = parseFloat(document.getElementById("ingresarMonto").value);
  document.querySelector(".grafica").style.display = "flex";
  if (!isNaN(montoClp)) {
